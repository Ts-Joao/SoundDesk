import type { StatusConfig, TrackStatus, Playlist } from "@/types";
...
export const getPlaylistProgress = (playlist: Playlist): number => {
  if (playlist.trackCount === 0) return 0;
  return Math.round((playlist.completedTracks / playlist.trackCount) * 100);
};
