...
class DownloadJobService:
    ...
    def download_playlist(self, playlist_id: UUID, user_id: UUID) -> int:
        ...
        playlist = PlaylistService(self.playlist_repository).find_by_id(
            playlist_id, user_id
        )
        ...
        try:
            for playlist_track in playlist.tracks:
                track_id = getattr(playlist_track, "track_id", playlist_track.id)
                track = self.repository.lock_track(track_id)
    ...
    def find_all(
        self, user_id: UUID, status: DownloadStatus | None = None
    ) -> list[DownloadJob]:
    ...
    def fail_track(self, track_id: UUID, error_message: str) -> None:
        self.repository.finish_active_jobs(
            track_id, DownloadStatus.FAILED, error_message[:2000]
        )
        self.repository.db.commit()
